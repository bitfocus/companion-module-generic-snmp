...
export class Generic_SNMP extends InstanceBase {
    ...
    pendingOids = new Set();
    oidTracker = new FeedbackOidTracker();
    snmpQueue = new PQueue({ concurrency: 1, interval: 10, intervalCap: 1 });
        ...
        if (this.config.traps) {
            try {
                ...
                if (err instanceof Error)
                    ...
                    this.log('error', `Could not initialize SNMP Trap listener: ${err}`);
            }
        }
        if (this.config.walk) {
    ...
    async createListener() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.listeningSocket.addListener('listening', () => {
                ...
                this.socketWrapper = new SharedUDPSocketWrapper(this.listeningSocket, this.config.portBind || 162, this.config.ip);
    ...
    processTrap(trap) {
        try {
            if (this.config.verbose)
                this.log(`debug`, `${snmp.PduType[trap.pdu.type]} recieved`);
            if (this.config.verbose)
                this.log('debug', JSON.stringify(trap));
    ...
    handleVarbind(varbind, index) {
        ...
        if ('value' in varbind && 'type' in varbind && varbind.type !== undefined) {
            if (varbind.type == snmp.ObjectType.NoSuchObject ||
                ...
                this.log('debug', `VarBind [${index}] OID: ${varbind.oid} type: ${snmp.ObjectType[varbind.type]}\nNot caching varbind`);
                return;
            }
            this.log('info', `VarBind [${index}] OID: ${varbind.oid} type: ${snmp.ObjectType[varbind.type]} value: ${varbind.value}`);
            ...
            if (this.feedbackIdsToCheck.size > 0)
    ...
    async setOid(oid, type, value) {
        ...
        await this.snmpQueue.add(async () => {
            return new Promise((resolve, reject) => {
                if (this.session == null)
                    ...
                    this.session.set([{ oid, type, value }], (error) => {
                        if (error) {
                            ...
                            if (this.config.verbose)
                                this.log('debug', `Set OID: ${oid} type: ${type} value: ${value}`);
                            resolve();
                        }
                    });
                }
    ...
    async sendInform(typeOrOid, varbinds = []) {
        return await this.snmpQueue.add(async () => {
            return new Promise((resolve, reject) => {
                ...
                if (this.session == null)
                    reject(new Error('SNMP session not initalized'));
                else {
                    ...
                    this.session.inform(typeOrOid, validatedVarBinds, (error) => {
                        ...
                        this.log('info', `Inform sent: ${typeof typeOrOid === 'number' ? snmp.TrapType[typeOrOid] : typeOrOid}${validatedVarBinds.length > 0 ? `\n${JSON.stringify(validatedVarBinds)}` : ''}`);
                        resolve();
                    });
                }
            });
            ...
            return new Promise((resolve, reject) => {
                ...
                if (this.session == null)
                    ...
                    this.session.trap(typeOrOid, validatedVarBinds, this.agentAddress, (error) => {
                        ...
                        this.log('info', `Trap sent: ${typeof typeOrOid === 'number' ? snmp.TrapType[typeOrOid] : typeOrOid}${validatedVarBinds.length > 0 ? `\n${JSON.stringify(validatedVarBinds)}` : ''}`);
                        resolve();
                    });
                }
            });
