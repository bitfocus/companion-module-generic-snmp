    ...
    switch (type) {
        case snmp.ObjectType.Boolean: {
            ...
            if (raw === 'true' || raw === '1' || raw === 'on' || raw === 'yes')
                value = true;
            else if (raw === 'false' || raw === '0' || raw === 'off' || raw === 'no')
                value = false;
            else
                throw new Error(`Cannot convert "${raw}" to Boolean — expected true/false, 1/0, on/off, yes/no`);
            break;
        }
        case snmp.ObjectType.Integer: {
            ...
            if (Number.isNaN(n) || (!isHex && String(n) !== raw.trim()))
                throw new Error(`Cannot convert "${raw}" to Integer`);
        ...
        case snmp.ObjectType.OID: {
            ...
            if (!isValidSnmpOid(trimmed))
                throw new Error(`Cannot convert "${raw}" to OID — invalid OID format`);
            value = trimmed;
            break;
        }
        case snmp.ObjectType.IpAddress: {
            ...
            if (!match)
                throw new Error(`Cannot convert "${raw}" to IpAddress — invalid IPv4 format`);
            const octets = match.slice(1).map(Number);
            if (octets.some((o) => o > 255))
                throw new Error(`IpAddress "${raw}" has an octet out of range [0, 255]`);
            value = raw;
            break;
        }
        ...
        case snmp.ObjectType.TimeTicks: {
            ...
            if (n < 0 || n > UINT32_MAX)
                throw new Error(`Value ${n} is out of range [0, ${UINT32_MAX}] for type ${type}`);
            value = n;
            break;
        }
        case snmp.ObjectType.Counter64: {
            ...
            if (n < 0n || n > UINT64_MAX)
                throw new Error(`Counter64 value ${n} is out of range [0, ${UINT64_MAX}]`);
...
export function prepareVarbindForVariableAssignment(varbind, displayString = false, divisor = 1) {
    const value = varbind.value;
    if (typeof value == 'number')
        return value / divisor;
    if (varbind.type == snmp.ObjectType.OctetString && displayString)
        return value?.toLocaleString() ?? '';
    if (varbind.type == snmp.ObjectType.Counter64 && Buffer.isBuffer(value))
        return bufferToBigInt(value).toString();
    if (typeof value == 'bigint')
        return value.toString();
    if (varbind.type == snmp.ObjectType.Opaque && Buffer.isBuffer(value))
...
//# sourceMappingURL=oidUtils.js.map